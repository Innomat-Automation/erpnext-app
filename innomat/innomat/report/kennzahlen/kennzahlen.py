...
def get_internal_data(filters):
    ...
    if len(ytd_revenue) > 1 and len(py_revenue) > 1 and 'total' in ytd_revenue[1] and 'total' in py_revenue[1] :
        data.append({
    ...
    if len(ytd_expenses) > 2 and len(py_expenses) > 2 and 'total' in ytd_expenses[-2] and 'total' in py_expenses[-2] :
        data.append({
