...
def get_totals_dict(account = ""):
	def _get_debit_credit_dict(label):
		return _dict(
			account="'{0} {1}'".format(label,account),
			debit=0.0,
			credit=0.0,
...
def initialize_gle_map(gl_entries, filters):
	...
	for gle in gl_entries:
		...
		if not account:
			...
			account = "<br>" + account
		gle_map.setdefault(gle.get(group_by), _dict(totals=get_totals_dict(account), entries=[]))
	return gle_map
...
def get_result_as_list(data, filters):
	...
	for d in data:
		...
		if balance == 0.0 and not d.get('account'): 
			d['balance'] = ""
		else:
			d['balance'] = balance
		...
		# Summe Credit und Debit bei Eröffunung und Schliessen entfernen, ist nur verwirrend über die Jahre. 
		if not d.get('account') or  d.get('account').startswith("'" + _('Opening')) or d.get('account').startswith("'" + _('Closing')) or d.get('account').startswith("'" + _('Closing (Opening + Total)')):
			d['debit'] = ""
			d['credit'] = ""
