...
def get_columns():
    return [
        {"label": _("Projekt"), "fieldname": "project", "fieldtype": "Link", "options": "Project", "width": 120},
        {"label": _("Fertig"), "fieldname": "finished", "fieldtype": "Check", "width": 60},
        {"label": _("Status"), "fieldname": "status", "fieldtype": "Select", "width": 80},
        {"label": _("Ampel"), "fieldname": "status_light", "fieldtype": "Data", "width": 60},
        {"label": _("Tasks"), "fieldname": "tasks", "fieldtype": "Integer", "width": 60},
        {"label": _("AB"), "fieldname": "sales_order", "fieldtype": "Link", "options": "Sales Order", "width": 80},
        {"label": _("Kunde"), "fieldname": "customer", "fieldtype": "Link", "options": "Customer", "width": 60},
        {"label": _("Kundenname"), "fieldname": "customer_name", "fieldtype": "Data", "width": 150},
        {"label": _("Volumen"), "fieldname": "volume", "fieldtype": "Currency", "width": 100},
        {"label": _("Akonto offen"), "fieldname": "akonto_amount", "fieldtype": "Currency", "width": 100},
        {"label": _("Akonto unbez."), "fieldname": "unpaid_akonto_amount", "fieldtype": "Currency", "width": 120},
        {"label": _("DN offen"), "fieldname": "dn_amount", "fieldtype": "Currency", "width": 100},
        {"label": _("DN draft"), "fieldname": "draft_dn_amount", "fieldtype": "Currency", "width": 100},
        {"label": _("TS offen"), "fieldname": "ts_amount", "fieldtype": "Currency", "width": 100},
        {"label": _("Total OP"), "fieldname": "total_op", "fieldtype": "Currency", "width": 100},
...
def get_data(filters):
    ...
    # projects that have open akonto / uninvoiced delivery notes / uninvoiced hours
    projects = frappe.db.sql(
        """SELECT
                ...
                SUM(`projects`.`total_op`) AS `total_op`,
           ...
           (
            ...
            UNION SELECT
               `tabSales Invoice`.`project` AS `project`,
                0 AS `akonto_amount`,
                0 AS `unpaid_akonto_amount`,
                0 AS `dn_amount`,
                0 AS `draft_dn_amount`,
                ...
                `tabSales Invoice`.`outstanding_amount` AS `total_op`
            FROM `tabSales Invoice`
            WHERE `tabSales Invoice`.`project` IS NOT NULL
              AND `tabSales Invoice`.`docstatus` = 1
           ...
           WHERE `projects`.`project` IS NOT NULL
             ...
             AND (akonto_amount > 0 OR unpaid_akonto_amount > 0 OR dn_amount > 0 OR draft_dn_amount > 0 OR ts_amount > 0 OR total_op > 0)
    ...
    # processing
    totals = {'volume': 0, 'akonto_amount': 0, 'unpaid_akonto_amount': 0, 'dn_amount': 0, 'draft_dn_amount': 0, 'ts_amount': 0, 'total_op': 0}
    for row in data:
        ...
        totals['total_op'] += row['total_op'] or 0
    data.append({
        'customer_name': "Total",
