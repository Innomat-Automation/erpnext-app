// Copyright (c) 2025, Frappe, libracore and Contributors

frappe.provide("frappe.ui.form");

frappe.ui.form.GridRow16 = class GridRow16 {
	constructor(opts) {
	...
	render_row(refresh) {
		...
		if(!this.row_index) {
			...
			this.row_index = $(
				`<div class="row-index sortable-handle col col-xx-1">
					${this.row_check_html}
		...
		if(this.doc && !this.grid.df.in_place_edit) {
			...
			if(!this.open_form_button) {
				this.open_form_button = $('<a class="close btn-open-row">\
					...
					.appendTo($('<div class="col col-xx-1"></div>').appendTo(this.row))
	...
	make_column(df, colsize, txt, ci) {
		...
		var $col = $('<div class="col grid-static-col col-xx-'+colsize+' '+add_class+'"></div>')
	...
	show_form() {
		if(!this.grid_form) {
			this.grid_form = new frappe.ui.form.GridRowForm16({
				row: this
			});
		}
