...
frappe.ui.form.Grid16 = class Grid16 {
	constructor(opts) {
		$.extend(this, opts);
		this.fieldinfo = {};
	...
	make_head() {
		// labels
		if(!this.header_row) {
			this.header_row = new frappe.ui.form.GridRow16({
	...
	refresh(force) {
		...
		for(var ri=0; ri < data.length; ri++) {
			...
			if(this.grid_rows[ri]) {
				...
				var grid_row = new frappe.ui.form.GridRow16({
					parent: $rows,
					parent_df: this.df,
					docfields: this.docfields,
					doc: d,
	...
	setup_visible_columns() {
		...
		// redistribute if total-col size is less than maximum
		var passes = 0;
		while(total_colsize < 15 && passes < 16) {
			for(var i in this.visible_columns) {
				var df = this.visible_columns[i][0];
				var colsize = this.visible_columns[i][1];
				if(colsize > 1 && colsize < 15
